#ifndef CHESSCXX_INCLUDE_CHESSCXX_BASE_FORMATTER_H_
#define CHESSCXX_INCLUDE_CHESSCXX_BASE_FORMATTER_H_
...
/// @brief Formatter for `std::optional<T>` where `T` is a `chesscxx` type that
/// supports formatting.
///
...
///
/// @tparam T A `chesscxx` type with `std::formatter<T>` defined.
template <typename T>
...
#endif  // CHESSCXX_INCLUDE_CHESSCXX_BASE_FORMATTER_H_
